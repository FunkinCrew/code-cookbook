...
# Migrating to the v0.9.0 assets system

The biggest and most obvious change modders will experience, is that the base game has reorganized the assets folder, which means that everything is in a new location.

Previously, assets were organized by file type (so images would go in the images folder), and then assets were further organized into "libraries" (where all the assets for Weekend 1 would go into a weekend1 folder). The asset library system would then reference a library, and try to preload assets from that library before loading the corresponding level. This had lots of problems; for example, characters were placed into a shared library, so the game would try to preload many unnecessary characters when loading a level.

The latest version of Funkin' foregoes this system, and is currently developing functions to preload only the assets a level actually uses. As part of this, we took on the long overdue task of organizing assets based on purpose, rather than by file type. Broadly, there are two main folders; ui contains assets used by the main menu and user interface (including the story and freeplay menus), excluding gameplay, while gameplay contains assets used during and after gameplay (including characters and stages, as well as game over and results screens). This means that, while the images, sounds, scripts, and data tied to a character in a song were previously located in separate folders, they are now all together!
```jsonc
// Here's where the old asset structure put character assets.
// Pretty much everything was in a different folder!
|-preload
 |-data
  |-characters
   |-bf.json
...
// And here's where the new asset structure puts character assets!
// A character's registry data, script, icon, main texture, and additional assets all go into the same folder.
|-gameplay
 |-characters
  |-bf
   |-bf-death
...
We also established lower-kebab-case as a standard convention for file names unless there's a specific reason for a file to be named a different way.

This WILL cause some issues for mods that will require migration... but fewer than you'd think! Any direct asset replacement mods probably won't work, and any mods which replace or inject a base game script will need fixing, these fixes are generally trivial and should just involve finding the new location of the file and moving it there. However, mods which add new songs, characters, stages, playable characters, remixes, etc. have been made BACKWARDS COMPATIBLE through copious amounts of tedious work coding magic! I (Eric) have tested against a bunch of the most popular mods, and have added code to explicitly provide backwards compatibility, minimizing the impact of this asset restructure.
That said, you should probably rearrange your mod's assets when you update, since we'll probably deprecate the backwards compatibility code eventually and the new asset structure is so much nicer anyway. Pro tip, you can just use an archive manager to extract an FNFC file into your mod's gameplay/songs folder and everything should just be in the right spot immediately, no need to take all the individual parts and fling them into different folders anymore!
