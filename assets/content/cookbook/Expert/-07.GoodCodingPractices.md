...
These access modifiers are allowed for any class field, whether static or not. The `public` modifier is used to denote that the field can be freely accessed by another class, whereas `private` fields can only be accessed by the class defining them and by its sub-classes.

This currently has no effect in HScript, since fields are public by default, even for non-scripted classes due to how Reflection works.

# Formatting Rules

Formatting rules are a set of standardized guidelines for structuring code. In this sub-article, we'll go over two common styles for styling your code.
