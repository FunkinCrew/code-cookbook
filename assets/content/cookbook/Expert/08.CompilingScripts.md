...
Scripts are always compiled with debug information, as it generates with the `CPPIA` format (rather than `CPPIB`, the binary format). This allows us to:
1. Collect debug information and provide that to you on crashes
2. Check for blacklisted class imports/fields.

So for security reasons, it is on - and the game will not accept anything other than `CPPIA`. The only real difference of course is size, and it is a bit larger than the binary format. Still, much easier to use.

## Targeting Platforms

*(must coincide with the `--target` argument if using platform specific native code. IE compiling a windows script should target `windows`, and a mac should target `macos`)*
...
### Extending CPPIA classes through HScript

You can define a class, like this (say a module):

```haxe
package kade.example;

import funkin.modding.module.Module;

@:hscriptExtendable
class MyCoolModule extends Module
{
    // ... code ...
}
```

Then in HScript you can extend it like so:
```haxe
import kade.example.MyCoolModule;

class ExtendedCool extends MyCoolModule
{
    // ... code, but extended! ...
}
...
<img width="2061" height="1257" alt="image" src="https://github.com/user-attachments/assets/b2e6c44d-3f83-4c2c-b2f8-c05a687c8d64" />

A great concern when using CPPIA scripts is that they are unreadable, inherently. As they're compiled to byte-code. Although it is possible to decompile a script, to see a representation of the source code that might've gone into making it. To see this in action, check out [CPPIADecompiler](https://github.com/FunkinCrew/CPPIADecompiler)
